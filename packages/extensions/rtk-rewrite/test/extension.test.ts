...
test("rtk rewrite extension defers direct Vitest commands to minimal-output", async () => {
  let handler:
    | ((event: unknown, ctx: { cwd: string; hasUI: boolean }) => Promise<void>)
    | undefined;
  ...
  const runner: RtkCommandRunner = async () => {
    runnerCalled = true;
    return { exitCode: 0, stderr: "", stdout: "rtk vitest" };
  };
  ...
  const event = {
    input: { command: "PATH=/tmp/fake:$PATH vitest run" },
    toolName: "bash",
  };
  ...
  assert.equal(event.input.command, "PATH=/tmp/fake:$PATH vitest run");
});
